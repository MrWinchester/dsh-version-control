// Part of the family shared surface (shared/host/mount-once.ts); kept in sync
// with the other packages via scripts/sync-shared.mjs.
/**
 * Host single-instance guard shared by the plugin family. The family bundle
 * (dsh-web-ui-all / dsh-skins) namespaces every child row id (web-ui-*), so
 * the loader accepts a standalone install of the same package side by side;
 * without this guard the second instance would still re-register the same
 * webserver routes, settings namespaces, and system-prompt sections and fail
 * the boot. mountOnce makes the second host apply a no-op for the lifetime of
 * the first instance (the browser half is already deduped by package name in
 * the client module host).
 *
